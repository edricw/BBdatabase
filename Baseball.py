...
transactionspicks['Draft_Year'] = transactionspicks['yearID']
transactionspicks['Team'] = transactionspicks['teamIDBR']
transactionspicks['Round'] = transactionspicks['draft-round']
transactionspicks = transactionspicks[['Full_Name', 'Draft_Year', 'Team', 'WAR', 'Round']]
transactionspicks = transactionspicks.round(3)

fig2 = go.Figure(data=[go.Table(header=dict(values=list(transactionspicks.columns)),
                                cells=dict(values=[transactionspicks.Full_Name, transactionspicks.Draft_Year,
                                                   transactionspicks.Team, transactionspicks.WAR,
                                                   transactionspicks.Round]))])
# fig2.show()
...
app.layout = html.Div(
    [html.H1(children="Welcome to the Baseball Stat Sheet!", style={'textAlign': 'center'}),
     html.Div([
         html.Div([
             html.Div([html.H3(children='WAR by Player Salary', style={'fontSize': 30, 'textAlign': 'center'}),
                       ...
                       dcc.Dropdown(id='team', options=[{'label': i, 'value': i} for i in team],
                               ...
                               'textAlign': 'center',
                               'width': '300px',
                               'margin': 'auto'})], style = {'height': '175px'}),
                       dcc.Graph(id='graph', style={'display': 'inline-block', 'height': '600px', 'width': "700px"})
                       ], className="six columns"),
             html.Div([
                 html.Div([html.H3(children='Wins by Payroll ', style={'fontSize': 30, 'textAlign': 'center'}),
                 html.Label('Year', style={'textAlign': 'center'}),
                 dcc.Dropdown(id='year2', options=[{'label': i, 'value': i} for i in year_id],
                         ...
                         'width': '300px',
                         'margin': 'auto',
                         'whiteSpace': 'pre-wrap'})], style = {'height': '175px'}),
                 dcc.Graph(id='random_graph', style={'display': 'inline-block', 'height': '600px', 'width': "700px"})
             ], className="six columns"),
         ], className="row"),
         html.Div(
             [html.H3(children='WAR by Draft Year', style={'fontSize': 30, 'textAlign': 'center'}),
